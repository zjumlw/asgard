...
/**
 * @Description 1743. 从相邻元素对还原数组 medium
...
public class Problem1743 {
    ...
    public int[] restoreArray(int[][] adjacentPairs) {
        ...
        for (int[] pair : adjacentPairs) {
            ...
            List<Integer> l1 = map.getOrDefault(num1, new ArrayList<>());
            l1.add(num2);
            map.put(num1, l1);

            List<Integer> l2 = map.getOrDefault(num2, new ArrayList<>());
            l2.add(num1);
            map.put(num2, l2);
        }
...
//        System.out.println(GsonUtil.toJson(map));
        ...
        for (int i = 2; i < ans.length; i++) {
            List<Integer> l = map.get(ans[i - 1]);
            ans[i] = ans[i - 2] == l.get(0) ? l.get(1) : l.get(0);
        }

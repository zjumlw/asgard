 ...
 * 题目测试用例保证 text 至少包含一个单词。请你重新排列空格，使每对相邻单词之间的空格数目都 相等 ，并尽可能 最大化 该数目。
...
public class Problem1592 {
    ...
    public String reorderSpaces(String text) {
            ...
            if (chars[i] == ' ') {
                ...
                for (int j = i; j < chars.length; j++) {
                    if (chars[j] == ' ') {
                        ...
                        String word = text.substring(i, chars.length);
                        words.add(word);
                        i = chars.length;
                        break;
                    }
        ...
        int spaceSize = spaceCnt / Math.max(wordCnt - 1, 1);
    ...
    public static void main(String[] args) {
        String text = "apple";
        int len = text.length();
        System.out.println(text.substring(1, len - 1));
    }
}

...
public class Problem1619 {
    ...
    public double trimMean(int[] arr) {
        ...
        for (int num : arr) {
            sum += num;
            if (minQueue.isEmpty() || minQueue.size() < cnt) {
                minQueue.add(num);
            } else if (num < minQueue.peek()) {
                minQueue.poll();
                minQueue.add(num);
            }
            if (maxQueue.isEmpty() || maxQueue.size() < cnt) {
                maxQueue.add(num);
        ...
        int minusSum = minQueue.stream().mapToInt(e -> e).sum() + maxQueue.stream().mapToInt(e -> e).sum();
        return 1.0 * (sum - minusSum) / (n - 2 * cnt);
    }

    public double trimMeanV2(int[] arr) {
        Arrays.sort(arr);
        int n = arr.length;
        int sum = 0;

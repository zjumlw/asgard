...
public class Problem526 {
    ...
    public int countArrangement(int n) {
        ...
        //System.out.println(GsonUtil.toJson(match));
        backtrack(1, n);
        return num;
    }
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int state = 0; state < mask; state++) {
                ...
                for (int k = 1; k <= n; k++) {
                    ...
                    // 数值 k 和 位置 i 之间要满足其中一种关系
                    if (k % i != 0 && i % k != 0) {
                        continue;
                    }
                    // state & (~(1 << (k - 1))) 代表将 state 中的数值 k 的位置置零
    ...
    public int countArrangementV3(int n) {
